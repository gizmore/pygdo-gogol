...
class GOGOL_Install:
    ...
    async def install(cls):
        await cls.install_player('gol')
        await cls.install_player('easy')
        await cls.install_player('medium')
        await cls.install_player('hard')

    @classmethod
    async def install_player(cls, difficulty: str):
        ...
        if not GDO_User.table().get_by_vals(conditions):
            ...
            await module_gogol.instance().save_config_val(f"gogol_ai_{difficulty}", ai.get_id())
